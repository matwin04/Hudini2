...
function clearRouteLines() {
    routeLayerIds.forEach((id) => {
        if (oldmap.getLayer(id)) oldmap.removeLayer(id);
    });

    routeSourceIds.forEach((id) => {
        if (oldmap.getSource(id)) oldmap.removeSource(id);
    });
...
function drawItineraryOnMap(itinerary) {
    ...
    if (hasCoords) {
        oldmap.fitBounds(bounds, { padding: 60, duration: 800 });
    }
}
...
function enableTransitPopups() {
    oldmap.on("mouseenter", "metro-stops-layer", () => {
        oldmap.getCanvas().style.cursor = "pointer";
    });

    oldmap.on("mouseleave", "metro-stops-layer", () => {
        oldmap.getCanvas().style.cursor = "";
    });

    oldmap.on("click", "metro-stops-layer", (e) => {
...
function setBaseLayer(layerName) {
    ...
    oldmap.setStyle(baseStyles[layerName]);

    oldmap.once("style.load", async () => {
        addTransitLayers();
...
async function reloadStopsLayer() {
    if (oldmap.getLayer("metro-stops-layer")) oldmap.removeLayer("metro-stops-layer");
    if (oldmap.getSource("stops")) oldmap.removeSource("stops");
...
function restoreTransitVisibility() {
    ...
    ["subway-lines", "rail-lines", "tram-lines", "metro-stops-layer"].forEach((id) => {
        if (oldmap.getLayer(id)) {
            oldmap.setLayoutProperty(id, "visibility", visibility);
        }
    });
}
...
function bindUI() {
    ...
    document.getElementById("zoomIn").addEventListener("click", () => oldmap.zoomIn());
    document.getElementById("zoomOut").addEventListener("click", () => oldmap.zoomOut());
    ...
    document.getElementById("toggle-stations").addEventListener("change", (e) => {
        if (oldmap.getLayer("metro-stops-layer")) {
            oldmap.setLayoutProperty("metro-stops-layer", "visibility", e.target.checked ? "visible" : "none");
        }
    });
    ...
    document.getElementById("toggle-rail").addEventListener("change", (e) => {
        ...
        ["subway-lines", "rail-lines", "tram-lines"].forEach((id) => {
            if (oldmap.getLayer(id)) oldmap.setLayoutProperty(id, "visibility", visibility);
        });
    });
